...
ggsave('Fig3B_1mm_noregress_1_5.eps', plot=p, width=6, height=4, units='in', dpi=300)
```
...
for (n in 1:400){
  ...
  derv <- derivatives(gam.model, unconditional = F) #interval = "simultaneous", 
...
ggplot(data = schaefer_H, mapping = aes(x = SArank, y = age)) +
  geom_point(aes(color = schaefer_H[['SArank']])) + ylim(-0.1, 0.3) +  xlim(0, 400) +
...
```{r setup, include=FALSE}
# Figure 3D
schaefer_H_extractp <- schaefer_H_extractp[schaefer_H_extractp$age>0, ]
schaefer_H_extractp[['SArank']] <- concat[['SArank']]
model <- gam(schaefer_H_extractp[['age']] ~ s(SArank), data = schaefer_H_extractp)
pred <- predict(model, se.fit=T)
ggplot(data = schaefer_H_extractp, mapping = aes(x = SArank, y = age)) +
  geom_point(aes(color = schaefer_H_extractp[['SArank']])) + ylim(0, 0.3) +  xlim(0, 400) +
  geom_ribbon(aes(ymin=pred$fit-1.96*pred$se.fit,
                   ymax=pred$fit+1.96*pred$se.fit), alpha = 1, fill = 'gray') +
  geom_smooth(method="gam", formula= y~s(x, k=k)) + scale_color_gradient(low='yellow', high='purple') +
  geom_line(size=.3, alpha = .8)  + theme(legend.position='none')
